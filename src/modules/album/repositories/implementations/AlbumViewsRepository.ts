...
class AlbumViewsRepository implements IAlbumViewsRepository {
  async listMostViewedAlbuns(limit: number): Promise<MostViewedAlbum[]> {
    return await prismaClient.$queryRawUnsafe<MostViewedAlbum[]>(
      "select  count(*) as numberOfViews,us.name ,us.lastname,us.email,al.title,al.description ,al.cover from album_views av inner join album al on av.albumId = al.id inner join users us on al.userId = us.id  group by albumId order by numberOfViews desc;"
    );
  }
